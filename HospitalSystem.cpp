...
void InputField::draw(sf::RenderWindow& w) {
    ...
    w.draw(box);
    char disp[512];
    if (isPwd) { for (int i = 0; i < len; i++) disp[i] = '*'; disp[len] = '\0'; }
    else { for (int i = 0; i <= len; i++) disp[i] = buf[i]; }
    txt.setString(disp);
    w.draw(txt);
}
...
HospitalManagementSystem::HospitalManagementSystem()
    : scr(SCR_ROLE), role(ROLE_NONE), logID(0), failCnt(0), locked(false),
    scrollY(0), activeField(nullptr), msgOn(false), msgErr(true),
    bkStep(0), bkDocID(0), bkDateFails(0),
    prStep(0), prApptID(0),
    histSearched(false), histPatID(0),
    selApptID(0), selBillID(0), selDocID(0), selPatID(0),
    secCount(0), reportReady(false),
    rptTotal(0), rptPend(0), rptComp(0), rptNos(0), rptCanc(0), rptRev(0.f)
{
    msgBuf[0] = '\0';
...
void HospitalManagementSystem::run() {
    ...
    while (win.isOpen()) {
        ...
        while (win.pollEvent(ev)) {
            ...
            if (ev.type == sf::Event::TextEntered && activeField &&
                ev.text.unicode < 128 &&
                ev.text.unicode != 8 && ev.text.unicode != 13 &&
                ev.text.unicode >= 32)
            ...
            if (ev.type == sf::Event::KeyPressed) {
                if (ev.key.code == sf::Keyboard::BackSpace && activeField)
                    activeField->pop();
                if (ev.key.code == sf::Keyboard::Escape) back();
            }
...
void HospitalManagementSystem::loadData() {
    try { fh.filePatients(storPat); }
    catch (...) {}
    try { fh.fileDoctor(storDoc); }
    catch (...) {}
    try { fh.fileAppointments(storApp); }
    catch (...) {}
    try { fh.fileAdmins(storAdm); }
    catch (...) {}
    try { fh.fileBills(storBill); }
    catch (...) {}
    try { fh.filePrescriptions(storPres); }
    catch (...) {}
}
...
void HospitalManagementSystem::getTodayStr(char* out) const {
    ...
    out[6] = '0' + yr / 1000; out[7] = '0' + (yr / 100) % 10;
    out[8] = '0' + (yr / 10) % 10; out[9] = '0' + yr % 10;
    out[10] = '\0';
}
...
bool HospitalManagementSystem::olderThan(const char* d, int days) {
    ...
    int yy = (d[6] - '0') * 1000 + (d[7] - '0') * 100 + (d[8] - '0') * 10 + (d[9] - '0');
    tm dt = {};
    dt.tm_mday = dd; dt.tm_mon = mm - 1; dt.tm_year = yy - 1900;
    time_t t0 = mktime(&dt);
...
void HospitalManagementSystem::goTo(AppScreen s) {
    scr = s; scrollY = 0;
    activeField = nullptr;
    for (int i = 0; i < 6; i++) flds[i].reset();
    clrMsg();
    selApptID = selBillID = selDocID = selPatID = 0;

    if (s == SCR_LOGIN) { fldID.reset(); fldPwd.reset(); }
    if (s == SCR_PAT_BOOK) { bkStep = 0; bkSpec[0] = bkDate[0] = bkSlot[0] = '\0'; bkDocID = 0; bkDateFails = 0; }
    if (s == SCR_DOC_PRESCRIBE) { prStep = 0; prApptID = 0; }
    if (s == SCR_DOC_HISTORY) { histSearched = false; histPatID = 0; }
    if (s == SCR_ADM_SECLOG) { loadSecLog(); }
    if (s == SCR_ADM_REPORT) { reportReady = false; computeReport(); }
}

void HospitalManagementSystem::logout() {
    role = ROLE_NONE; logID = 0;
    failCnt = 0; locked = false;
    goTo(SCR_ROLE);
}

void HospitalManagementSystem::back() {
    switch (scr) {
    case SCR_LOGIN:         goTo(SCR_ROLE);     break;
    case SCR_PAT_MENU:      logout();            break;
    case SCR_PAT_BOOK:
    case SCR_PAT_CANCEL:
    case SCR_PAT_APPTS:
    case SCR_PAT_RECORDS:
    case SCR_PAT_BILLS:
    case SCR_PAT_PAY:
    case SCR_PAT_TOPUP:     goTo(SCR_PAT_MENU); break;
    case SCR_DOC_MENU:      logout();            break;
    case SCR_DOC_TODAY:
    case SCR_DOC_COMPLETE:
    case SCR_DOC_NOSHOW:
    case SCR_DOC_PRESCRIBE:
    case SCR_DOC_HISTORY:   goTo(SCR_DOC_MENU); break;
    case SCR_ADM_MENU:      logout();            break;
    default:                goTo(SCR_ADM_MENU);  break;
    }
}
...
bool HospitalManagementSystem::drawBtn(const char* lbl, float x, float y, float w, float h,
    sf::Color fill, sf::Vector2f mouse, sf::Color tc) {
...
void HospitalManagementSystem::drawBg() {
    ...
    for (int yy = 0; yy < H; yy += 40) { hl.setPosition(0, yy); win.draw(hl); }
    sf::RectangleShape vl({ 1, (float)H });
    vl.setFillColor(sf::Color(20, 32, 55, 55));
    for (int xx = 0; xx < W; xx += 40) { vl.setPosition(xx, 0); win.draw(vl); }
}
    ...
    if (role != ROLE_NONE && logID != 0) {
        ...
        if (role == ROLE_PAT) {
            ...
            if (p) {
                ...
                strApp(info, "  |  PKR ");
                char b[32]; ftoStr(p->getBalance(), b);
                strApp(info, b);
            }
        }
        else if (role == ROLE_DOC) {
            Doctor* d = storDoc.findByID(logID);
            if (d) { strApp(info, "Dr. "); strApp(info, d->getName()); }
        }
        else {
...
void HospitalManagementSystem::onClick(sf::Vector2f p) {
    switch (scr) {
    case SCR_ROLE:          onRoleClick(p);        break;
    case SCR_LOGIN:         onLoginClick(p);       break;
    case SCR_PAT_MENU:      onPatMenuClick(p);     break;
    case SCR_PAT_BOOK:      onPatBookClick(p);     break;
    case SCR_PAT_CANCEL:    onPatCancelClick(p);   break;
    case SCR_PAT_PAY:       onPatPayClick(p);      break;
    case SCR_PAT_TOPUP:     onPatTopupClick(p);    break;
    case SCR_DOC_MENU:      onDocMenuClick(p);     break;
    case SCR_DOC_COMPLETE:  onDocCompleteClick(p); break;
    case SCR_DOC_NOSHOW:    onDocNoshowClick(p);   break;
    case SCR_DOC_PRESCRIBE: onDocPrescribeClick(p); break;
    case SCR_DOC_HISTORY:   onDocHistoryClick(p);  break;
    case SCR_ADM_MENU:      onAdmMenuClick(p);     break;
    case SCR_ADM_ADD_DOC:   onAdmAddDocClick(p);   break;
    case SCR_ADM_REM_DOC:   onAdmRemDocClick(p);   break;
    case SCR_ADM_DISCHARGE: onAdmDischargeClick(p); break;
        /* read-only / back-only screens handled via ESC or Back button */
    case SCR_PAT_APPTS:
    case SCR_PAT_RECORDS:
    case SCR_PAT_BILLS:
    case SCR_DOC_TODAY:
    ...
    case SCR_ADM_SECLOG:
    case SCR_ADM_REPORT:
        if (btnHit(40, H - HDR - FTR - 42, 90, 36, p)) back();
        break;
    default: break;
    }
}
...
void HospitalManagementSystem::onRoleClick(sf::Vector2f p) {
    float bw = 300, bh = 62, bx = (W - bw) * .5f, by = 272, gap = 74;
    if (btnHit(bx, by + 0 * gap, bw, bh, p)) { role = ROLE_PAT; goTo(SCR_LOGIN); }
    else if (btnHit(bx, by + 1 * gap, bw, bh, p)) { role = ROLE_DOC; goTo(SCR_LOGIN); }
    else if (btnHit(bx, by + 2 * gap, bw, bh, p)) { role = ROLE_ADM; goTo(SCR_LOGIN); }
    else if (btnHit(bx, by + 3 * gap, bw, bh, p)) win.close();
}
...
void HospitalManagementSystem::drawLogin(sf::Vector2f m) {
    ...
    if (failCnt > 0) {
        char fc[64] = "Failed attempts: "; char n[8]; itoStr(failCnt, n); strApp(fc, n);
...
void HospitalManagementSystem::onLoginClick(sf::Vector2f p) {
    if (locked) return;
    if (fldID.hit(p)) { fldID.active = true;  fldPwd.active = false; activeField = &fldID;  return; }
    if (fldPwd.hit(p)) { fldPwd.active = true; fldID.active = false;  activeField = &fldPwd; return; }
    if (btnHit(440, 450, 88, 46, p)) { back(); return; }
    if (btnHit(542, 450, 200, 46, p)) {
        int id = (int)stringToFloat(fldID.buf);
        if (!Validator::validateID(id)) { setMsg("Please enter a valid numeric ID."); return; }
        if (tryLogin(id, fldPwd.buf)) {
            logID = id; failCnt = 0; clrMsg();
            if (role == ROLE_PAT) goTo(SCR_PAT_MENU);
            else if (role == ROLE_DOC) goTo(SCR_DOC_MENU);
            else                       goTo(SCR_ADM_MENU);
        }
        else {
            failCnt++;
            const char* rs = role == ROLE_PAT ? "Patient" : role == ROLE_DOC ? "Doctor" : "Admin";
            fh.writeSecurityLog(rs, id, "FAILED");
            if (failCnt >= 3) { locked = true; setMsg("Account locked. Contact admin."); }
            else setMsg("Invalid ID or password. Try again.");
        }
    }
}

bool HospitalManagementSystem::tryLogin(int id, const char* pwd) {
    if (role == ROLE_PAT) { Patient* p = storPat.findByID(id); return p && isEqual(p->getPass(), pwd); }
    if (role == ROLE_DOC) { Doctor* d = storDoc.findByID(id); return d && isEqual(d->getPass(), pwd); }
    Admin* a = storAdm.findByID(id); return a && isEqual(a->getPass(), pwd);
}
...
void HospitalManagementSystem::drawPatMenu(sf::Vector2f m) {
    ...
    if (p) { char b[32]; ftoStr(p->getBalance(), b); strApp(sub, b); }
    ...
    for (int i = 0; i < 8; i++) drawBtn(lbls[i], bx, by + i * gap, bw, bh, cols[i], m);
}
...
void HospitalManagementSystem::onPatMenuClick(sf::Vector2f p) {
    ...
    for (int i = 0; i < 7; i++)
        if (btnHit(bx, by + i * gap, bw, bh, p)) { goTo(scrs[i]); return; }
    if (btnHit(bx, by + 7 * gap, bw, bh, p)) logout();
}
...
void HospitalManagementSystem::drawPatBook(sf::Vector2f m) {
    ...
    if (bkStep == 0) {
        ...
        flds[0].setup(font, lx, cy + 26, 480, 44); flds[0].draw(win);
        ...
        char stitle[96] = "Doctors in: "; strApp(stitle, bkSpec);
        ...
        if (bkDocID > 0) drawBtn("Continue", lx + 100, ay, 140, 38, sf::Color(0, 122, 80), m);
        if (bkDocID > 0) {
            char sel[80] = "Selected: ";
            Doctor* dd = storDoc.findByID(bkDocID);
            if (dd) { strApp(sel, dd->getName()); }
        ...
        flds[1].setup(font, lx, cy + 62, 260, 44); flds[1].draw(win);
...
void HospitalManagementSystem::onPatBookClick(sf::Vector2f p) {
    ...
    if (bkStep == 0) {
        if (flds[0].hit(p)) { flds[0].active = true; activeField = &flds[0]; return; }
        if (btnHit(lx, cy + 100, 90, 38, p)) { back(); return; }
        if (btnHit(lx + 490, cy + 26, 160, 44, p)) {
            if (getLength(flds[0].buf) == 0) { setMsg("Please enter a specialization."); return; }
            copyStr(bkSpec, flds[0].buf);
            bool found = false;
            for (int i = 0; i < storDoc.getSize(); i++) {
                char ds[64], bs[64];
        ...
        if (flds[1].hit(p)) { flds[1].active = true; activeField = &flds[1]; return; }
        if (btnHit(lx, cy + 138, 90, 38, p)) { bkStep = 1; return; }
        if (btnHit(lx + 270, cy + 62, 240, 44, p)) {
            if (!Validator::validateDate(flds[1].buf)) {
                bkDateFails++;
...
void HospitalManagementSystem::doBookAppt() {
    ...
    storApp.add(a); fh.appendAppointment(a);
    Bill b(bid, logID, aid, doc->getFee(), "unpaid", today);
    storBill.add(b); fh.appendBill(b);
    fh.savePatients(storPat);
...
void HospitalManagementSystem::doCancelAppt(int apptID) {
    ...
    a->setStatus("cancelled"); fh.saveAppointment(storApp);
    for (int i = 0; i < storBill.getSize(); i++)
        if (storBill[i].getAppointmentID() == apptID) { storBill[i].setStatus("cancelled"); break; }
    fh.saveBills(storBill);
    Patient* pat = storPat.findByID(logID);
    if (pat) { *pat += fee; fh.savePatients(storPat); }
...
void HospitalManagementSystem::doPayBill(int billID) {
    ...
    b->setStatus("paid");
    fh.saveBills(storBill); fh.savePatients(storPat);
...
void HospitalManagementSystem::drawPatTopup(sf::Vector2f m) {
    ...
    flds[0].setup(font, px + 20, py + 70, 400, 44); flds[0].draw(win);
...
void HospitalManagementSystem::onPatTopupClick(sf::Vector2f p) {
    ...
    if (btnHit(px + 20, py + 128, 175, 44, p)) {
        float amt = stringToFloat(flds[0].buf);
...
void HospitalManagementSystem::doMarkComplete(int apptID) {
    Appointment* a = storApp.findByID(apptID);
    if (!a) { setMsg("Appointment not found."); return; }
    a->setStatus("completed"); fh.saveAppointment(storApp);
...
void HospitalManagementSystem::doMarkNoshow(int apptID) {
    Appointment* a = storApp.findByID(apptID);
    if (!a) { setMsg("Appointment not found."); return; }
    a->setStatus("no-show"); fh.saveAppointment(storApp);
    for (int i = 0; i < storBill.getSize(); i++)
        if (storBill[i].getAppointmentID() == apptID) { storBill[i].setStatus("cancelled"); break; }
    fh.saveBills(storBill);
...
void HospitalManagementSystem::drawDocPrescribe(sf::Vector2f m) {
    ...
    if (prStep == 0) {
        ...
        flds[0].setup(font, lx, cy + 26, 280, 44); flds[0].draw(win);
        ...
        flds[1].setup(font, lx, cy + 52, 900, 44); flds[1].draw(win);

        drawLbl("Notes:", lx, cy + 108);
        flds[2].setup(font, lx, cy + 128, 900, 44); flds[2].draw(win);
...
void HospitalManagementSystem::onDocPrescribeClick(sf::Vector2f p) {
    ...
    if (prStep == 0) {
        ...
        if (btnHit(lx + 290, cy + 26, 120, 44, p)) {
            int aid = (int)stringToFloat(flds[0].buf);
        ...
        if (flds[1].hit(p)) { flds[1].active = true; flds[2].active = false; activeField = &flds[1]; return; }
        if (flds[2].hit(p)) { flds[2].active = true; flds[1].active = false; activeField = &flds[2]; return; }
...
void HospitalManagementSystem::doWritePrescription() {
    if (getLength(flds[1].buf) == 0) { setMsg("Please enter medicines."); return; }
    Appointment* a = storApp.findByID(prApptID);
    if (!a) { setMsg("Appointment not found."); return; }
    char today[16]; getTodayStr(today);
    int pid = nextPresID();
    Prescription pr(pid, prApptID, a->getPatID(), logID, today, flds[1].buf, flds[2].len > 0 ? flds[2].buf : "N/A");
    storPres.add(pr); fh.appendPrescription(pr);
...
void HospitalManagementSystem::drawDocHistory(sf::Vector2f m) {
    ...
    flds[0].setup(font, lx, cy + 24, 240, 44); flds[0].draw(win);
...
void HospitalManagementSystem::onDocHistoryClick(sf::Vector2f p) {
    ...
    if (btnHit(lx + 250, cy + 24, 120, 44, p)) {
        int id = (int)stringToFloat(flds[0].buf);
...
void HospitalManagementSystem::drawAdmAddDoc(sf::Vector2f m) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        flds[i].setup(font, lx, cy + 20 + i * 68, 500, 40);
        flds[i].draw(win);
    }
...
void HospitalManagementSystem::onAdmAddDocClick(sf::Vector2f p) {
    ...
    for (int i = 0; i < 5; i++)
        if (flds[i].hit(p)) {
            for (int j = 0; j < 5; j++) flds[j].active = false;
            flds[i].active = true; activeField = &flds[i]; return;
        }
...
void HospitalManagementSystem::doAddDoctor() {
    if (getLength(flds[0].buf) == 0) { setMsg("Name cannot be empty."); return; }
    if (getLength(flds[1].buf) == 0) { setMsg("Specialization cannot be empty."); return; }
    if (!Validator::validateContact(flds[2].buf)) { setMsg("Contact must be exactly 11 digits."); return; }
    if (!Validator::validatePassword(flds[3].buf)) { setMsg("Password must be at least 6 characters."); return; }
    float fee = stringToFloat(flds[4].buf);
    if (!Validator::validatePositiveFloat(fee)) { setMsg("Fee must be a positive number."); return; }

    int newID = nextDocID();
    char sid[16]; itoStr(newID, sid);
    Doctor d(flds[1].buf, fee, newID, flds[0].buf, flds[2].buf, flds[3].buf);
    storDoc.add(d); fh.appendDoctor(d);
...
void HospitalManagementSystem::doRemoveDoctor(int docID) {
    ...
    storDoc.removeByID(docID); fh.saveDoctor(storDoc);
...
void HospitalManagementSystem::drawAdmDischarge(sf::Vector2f m) {
    ...
    flds[0].setup(font, px + 20, py + 92, 380, 44); flds[0].draw(win);
...
void HospitalManagementSystem::doDischarge(int patID) {
    ...
    fh.dischargePatient(patID, storPat, storApp, storBill, storPres);
    flds[0].reset(); setMsg("Patient discharged and archived successfully.", false);
}
