...
enum UserRole { 
    ROLE_NONE = 0, ROLE_PAT, ROLE_DOC, ROLE_ADM 
};
...
struct InputField {
    ...
    InputField() : len(0), active(false), isPwd(false) {
        *(buf+0) = '\0';
    }
    void reset() {
        *(buf + 0) = '\0';
...
class HospitalManagementSystem {
    ...
    //helpers for  storage queries
    int  nextApptID();
    int  nextBillID();
    int  nextPresID();
    int  nextDocID();
    bool slotTaken(int docID, const char* date, const char* slot);
    //helpers for sorting (used bubble sort here)
